...
<?php
    $themeSetup = array(
        ...
        'header' => get_theme_mod('coders_theme_fixed_header','')
    );
    //var_dump($themeSetup);
?>
    <body <?php body_class(); ?>>
        <header id="site-header" class="header-group <?php print $themeSetup['header'] ?>">
            <?php if( strlen($themeSetup['top_bar']) ) : ?>
            <div class="top-bar container <?php print $themeSetup['top_bar'] ?>">
                <div class="wrap clearfix">
                    <?php coders_theme_part( 'top-bar-' . $themeSetup['top_bar'] ); ?>
                </div>
            </div>
            <?php endif; ?>
        ...
        <main id="site-content" class="container">
                ...
                <?php if( $post_type[0] === FALSE || is_404() ) : ?> 
                    ...
                    <?php coders_theme_part( '404' ); ?>
                    </div>
                <?php elseif( $post_type[0] === 'page' ) : ?>
                <div class="content page">
                    <?php coders_theme_part( 'page' ); ?>
                </div>
                <?php else: ?>
                <div class="container content <?php print implode(' ', $post_type);  ?>">
                    <?php coders_theme_part( implode('-', $post_type) ); ?>
                </div>
                <?php endif; ?>
        </main><!-- #site-content -->
        ...
        <footer id="site-footer" class="footer-group">
            ...
            <div class="bottom-bar">
                ...
                <!-- bottom bar -->
                    <?php coders_theme_part( 'bottom-bar-' . $themeSetup['bottom_bar'] ); ?>
                </div>
            </div><!-- .section-inner -->
            <?php endif; ?>
        </footer><!-- #site-footer -->	
